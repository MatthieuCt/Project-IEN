 ...
 */
if(typeof(allowSelectWeekend) == "undefined")
	allowSelectWeekend = true; 
if(typeof(limitedDate) == "undefined")
	limitedDate = false;
...
var DayCol = new Array("Pn", "Wt", "Sr", "Cz", "Pt", "So", "Nd");
var MonthCol = new Array("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov", "Dec")
var MonthColFull = new Array("Stycze&#324;","Luty","Marzec","Kwiecie&#324;","Maj","Czerwiec",
...
var cw = {
	...
	getConfDays: function()
	{
		...
		for(i=1;i<=getDaysInMonth(this.selMonth, this.selYear); i++)
		{
          ...
          if( allowSelectWeekend ){
            // mozliwosc wyboru weekendow
            dayLink.href="javascript: newDay("+ i + ");fillBackDate("+i+","+this.selMonth+","+ this.selYear+")";
          }
        }else{
          if( limitedDate ){
            // ograniczenie wyboru do okresu
            var dateIn = new Date( this.selYear , this.selMonth, i );  
            if( isallowDate( dateIn ) ){
              dayCell.className = "dayAllow";
              dayLink.href="javascript: newDay("+ i + ");fillBackDate("+i+","+this.selMonth+","+ this.selYear+")";
              
            }else{
              dayLink.className = "dayNotAllow"; 
            }
	...
	setBrowseYears: function()
	{
		...
		else
		{
			backB= cel('a');
			backB.id = "prevBtn";
			backB.href = "javascript:modMonth("+this.selYear+","+this.selMonth+"-1)";
			//
		}
		...
		yText.id = 'yearLabel'
		yText.align = 'center'
		yText.innerHTML = cw.selYear +", "+ MonthColFull[ cw.selMonth ];
		...
		if (this.selYear >= this.endYear && this.config == "strict") {
			...
			//
			fwdB.id = "nextBtn";
			fwdB.href = "javascript: modMonth(" + this.selYear + "," + this.selMonth + "+1)";
		//
		//
		//
		//
...
function createCalendarElements()
{
	...
	clearBtna = cel('a');
	clearBtna.href = "javascript: clearCalendar()";
	clearBtna.innerHTML = "Wyczyść";
	appendc(clearBtn, clearBtna);
	appendc(document.getElementById(el), clearBtn);
}
...
/** main function */
function fPopCalendarStrict(objDateMin, objDateMax,param1, param2, param3){
  allowSelectWeekend = false;
  limitedDate = true;
  
  minDate = document.getElementById(objDateMin).value;
  maxDate = document.getElementById(objDateMax).value;
  
  fPopCalendarCore(param1, param2, param3)
}
...
function fPopCalendarLeft(param1, param2, param3){
  allowSelectWeekend = true;
  limitedDate = false;
  folowMouse = false
...
/** main function */
function fPopCalendarCore(param1, param2, param3)
{
 ...
 *  function 
 */ 
function isallowDate(date) {
  // date is a JS Date object
	if( this.limitedDate ){
    
    var date_min = this.minDate;
  	var arrDateMin = getArrayDate(date_min);
  	var DateMin = new Date(Number(arrDateMin[2]), Number(arrDateMin[0]-1), Number(arrDateMin[1]) );
  															  
  	var date_max = this.maxDate;
  	var arrDateMax = getArrayDate(date_max);
  	var DateMax = new Date(Number(arrDateMax[2]), Number(arrDateMax[0]-1), Number(arrDateMax[1]) );
...
function getArrayDate(strDate){
   	...
   	var retArr = new Array(  Number(arr[1]) , Number(arr[2]) , Number( arr[0] ) );
		return retArr;														
